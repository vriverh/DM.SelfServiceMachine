...
namespace DM.LocalServices.API
{
    ...
    public class DMApi
    {
            ...
            while (configuration.GetValue<bool>("Regist"))
            {
                try
                {
                    ...
                    if (resultMessage.Code == 200)
                    {
                        ...
                        // Removed WPF UI dependency - this is now an API-only service
                        SetWinInfo();
                    }
                    else
                    {
        ...
        {
            // TODO: Implement configuration update mechanism for the WebView app
            // This API service no longer has a direct UI, but it could send configuration
            // to the WebView application through IPC or shared configuration files
            logger.LogDebug("Configuration updated - URL: {Url}, State: {State}", url, state);
            ...
            if (configuration.GetValue<bool>("Regist"))
            {
                ...
                try
                {
                    ...
                    if (resultMessage.Code == 200)
                    {
                        ...
                        // Removed WPF dispatcher dependency
